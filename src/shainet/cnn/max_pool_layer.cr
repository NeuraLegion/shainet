  ...
  class Max_pool_layer
    ...
    def activate
      @input_data.size.times do |channel|      
        ...
        until input_y == (@input_data[channel].size - @pool - 1)
          until input_x == (@input_data[channel][input_y].size - @pool - 1)
            window = @input_data[channel][input_y..(input_y + @pool/2 - 1)].map { |m| m[input_x..(input_x + @pool/2 - 1)] }
