...
/*
 * sgemm_cpu: C[M, N] = A[M, K] * B[K, N]
 *
 * All three are row-major fp32. Uses AVX2 FMA with tiling for L2 cache
 * locality, and OpenMP for N-parallelism.
 *
 * This is used for the dequanted fp32 Q+gate weights in full-attention
 * layers, which would otherwise run through Crystal's single-threaded
 * matmul.
 */
